...
| `tempest_fastapi_sdk.genai` *(extra: `[genai]`; capacity check imports without it)* | Self-hosted GenAI: capacity check (`probe_hardware`/`HardwareInfo`, `can_run`/`recommend`/`CapacityReport`, `estimate_model_bytes`/`fetch_num_params`, `ModelDtype`); local LLM `TextGenerator` (`generate`/`chat`/`stream`, int4/int8 quant, lazy load + idle unload); `Embedder` (+ `EmbeddingCache`/`InMemoryEmbeddingCache`, `normalize=`) + `cosine_similarity`, `BatchScheduler` (coalesce concurrent calls), `ModelRegistry` (LRU model sharing); `resolve_device`/`auto_dtype_name` |
| `tempest_fastapi_sdk.genai.rag` *(extra: `[genai-rag]`)* | RAG context for local LLMs: `WebSearch` (`.retrieve` one-shot search→extract→context) / `SearxngBackend` / `WebSearchBackend` + `SearchResult`, `ContentExtractor` (trafilatura, `.extract_many` concurrent), `PdfReader` (PyMuPDF) + `Document`/`Chunk`/`PdfPage`, `chunk_text`, `build_context`; corpus RAG — `Retriever` + `VectorStore`/`InMemoryVectorStore`/`PgVectorStore` (pgvector); audio (`[genai-audio]`) — `SpeechToText` (faster-whisper) + `TextToSpeech` (Coqui TTS) + `Transcription` |
| `tempest_fastapi_sdk.geo` *(extra: `[geo]`; heuristic imports without it)* | Geolocation without a paid API: `haversine_km` (great-circle distance), `estimate_travel` (offline road distance + per-mode time), `OSRMBackend`/`RoutingBackend` (real routing via free OSRM), `Coordinate`/`TravelEstimate`/`TravelMode`, `DEFAULT_MODE_DURATION_FACTORS`/`DEFAULT_CIRCUITY_FACTOR`/`DEFAULT_CAR_SPEED_KMH` |
