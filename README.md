...
| `tempest_fastapi_sdk.agents` | AI agents over the models you already self-host: `Agent` (goal in, traced `AgentRun` out — `run` / `stream`), `AgentBudget` (steps + wall-clock + tool calls, with `StopReason` saying which ceiling fired), `AgentTool` / `AgentContext` / `ToolResult` / `AgentToolError` (two-arg handlers; a raising tool becomes an observation for the model, never a crashed run), `AgentArtifact` (binary results chained by name, so an agent can draw an image and then have the VLM look at it without disk or base64), ready-made tools over the local models — `generate_image_tool`, `describe_image_tool`, `transcribe_audio_tool`, `speak_tool`, `retrieve_tool`, `web_search_tool`, `save_artifact_tool`, `text_tool` — plus persistence that is opt-in (`InMemoryAgentRunSink` bounded buffer, `BaseAgentRunModel`/`make_agent_run_model`/`DbAgentRunSink`) and `make_agent_router` (`POST /run`, `/run/stream` SSE, artifact download) |
