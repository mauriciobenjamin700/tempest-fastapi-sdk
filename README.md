...
| `tempest_fastapi_sdk.openpix` | `OpenPixEnvironment`, `OpenPixEvent` (28 webhook events), `make_openpix_webhook_dependency` / `webhook_verifier` / `decode_public_key`, `to_cents` / `reais_to_cents` / `cents_to_reais` — the thin typed layer over an integration generated by `tempest openapi-client`; the generated schemas stay in your service |
