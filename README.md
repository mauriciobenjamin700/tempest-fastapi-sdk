...
- **Começando do zero / New to Python tooling:** [Instalar o uv](https://mauriciobenjamin700.github.io/tempest-fastapi-sdk/getting-started/uv/) · [Install uv](https://mauriciobenjamin700.github.io/tempest-fastapi-sdk/en/getting-started/uv/) — trilha para iniciantes: `uv`, versões do Python, primeiro projeto e documentações oficiais.
...
Shared FastAPI/SQLAlchemy/Pydantic building blocks used across Tempest projects: base schemas, ORM model, async repository, pagination, settings, exceptions, Alembic helper, FastStream/TaskIQ broker managers, Redis cache, Server-Sent Events, Web Push, a Django-style **admin site** (`AdminSite` + `AdminModel`), **typed SSR pages** (`Page` + `html_response`, HTMX-ready), **typed forms generated from Pydantic schemas** and **typed CSS** (`form_for` + `StyleSheet`), and the utility classes (`PasswordUtils`, `JWTUtils`, `EmailUtils`, `UploadUtils`, `DownloadUtils`, `FileStoreUtils`, `MetricsUtils`, `LogUtils`).
...
| `[webauthn]` | `fido2` | Passkeys / security keys — `WebAuthnService`, `make_web_authn_credential_model`, `/auth/webauthn/*`: passwordless, phishing-resistant login |
| `[firebase]` | `firebase-admin` | Firebase ID token verification — `FirebaseAuth` (idempotent app init, `get_identity` / `get_uid` / `get_optional_identity`), `FirebaseIdentity`, `FirebaseUserResolver`, `FirebaseSettings`; one error `code` per failure. Heavy (33 packages, 52 MB measured on firebase-admin 7.5.0), lazily imported, out of `[all]` |
| `[vision]` | `ort-vision-sdk` | `Detector` / `Classifier` / `Segmenter` / `DetectClassify` (ONNX) + prediction schemas |
| `[modelops]` | `psutil`, `nvidia-ml-py`, `numpy` | Benchmark any callable — latency (median/IQR/p95/p99), RAM, GPU and energy (`benchmark`, `NvmlPowerSampler`, `RaplEnergySampler`); drift monitoring (`baseline_from_samples`, `PredictionMonitor`) and reading the compact format (`read_compact`, `predict_compact`) |
| `[modelops-compose]` | `ort-vision-sdk[compose]` | Fuse a detector and a classifier into a single `.onnx` graph — `fuse_detect_classify`. The dynamic crop between the stages is bridged with `RoiAlign`, which is why `onnx.compose.merge_models` alone does not do this. Build-time only; serving the fused graph is `[vision]` |
| `[modelops-onnx]` | `onnx`, `onnxruntime` | Static analysis, ONNX benchmarking, `.onnx` → `.ort` conversion, graph optimization and int8 quantization |
...
| `[genai-audio]` | `faster-whisper`, `coqui-tts`, `torch`, `torchaudio`, `torchcodec`, `transformers<5` | Self-hosted voice — `SpeechToText` (STT) + `TextToSpeech` (TTS) + `Language` presets (PT-BR/EN-US) |
...
| `[faces]` | `onnxruntime`, `pillow`, `numpy` | Face detection and recognition on ONNX Runtime (no opencv, no torch, no system libraries) — `FaceRecognizer`, `compare_faces`, 16 MB default pack |
...
| Module | Exports |
| --- | --- |
| `tempest_fastapi_sdk.admin` *(extra: `[admin]`)* | `AdminSite`, `AdminModel` (incl. `audit_model=...` for a per-row audit-history timeline, `autocomplete_fields=[...]` for HTMX-searched FK inputs, `inlines=[Inline(...)]` for 1-N children on the detail view, `Inline(editable=True, can_delete=True)` for in-place formset editing of those children), `Inline`, dashboard business-metric cards (`AdminSite(dashboard_cards=[...])`: `MetricCard` + `MetricValue` / `MetricTrend` / `MetricPartition`), CSV import (`AdminModel(can_import=True)`), granular RBAC (`make_admin_router(access_policy=...)` + `AdminPermission`), lenses (`AdminModel(lenses=[Lens(...)])` saved list presets), JSON + time field widgets, `make_admin_router`, `AdminAuthBackend`, `UserModelAuthBackend`, `AdminAuthError` |
| `tempest_fastapi_sdk.agents` | AI agents over the models you already self-host: `Agent` (goal in, traced `AgentRun` out — `run` / `stream`), `AgentBudget` (steps + wall-clock + tool calls, with `StopReason` saying which ceiling fired), `AgentTool` / `AgentContext` / `ToolResult` / `AgentToolError` (two-arg handlers; a raising tool becomes an observation for the model, never a crashed run), `AgentArtifact` (binary results chained by name, so an agent can draw an image and then have the VLM look at it without disk or base64), ready-made tools over the local models — `generate_image_tool`, `describe_image_tool`, `transcribe_audio_tool`, `speak_tool`, `retrieve_tool`, `web_search_tool`, `save_artifact_tool`, `text_tool` — **test helpers** (`agents.testing` — `ScriptedBackend` scripts the model's decisions so an agent is unit-testable with no model, plus `FailingBackend` and `assert_completed`/`assert_used_tools`/`assert_artifact`), **three memory layers** (`scratchpad_tools` within a run, `fact_tools`/`FactStore` durable and editable per `subject`, with `DbFactStore` and `RedisFactStore` for stores that survive a restart, `recall_prompt` semantic over `ChatMemory` — facts are exact and correctable, recall is fuzzy and unauditable, and the recipe leads with which to pick), **skills** (`Skill` loaded on demand — only name + one-line description sit in the prompt, the full instructions and the skill's own tools arrive when the model calls `load_skill`; `discover_skills` reads `SKILL.md` files in Claude Code's format), **typed tools** (`@tool` deriving the JSON schema from a Pydantic args model, so a bad argument becomes a correctable observation instead of a `KeyError`), **structured output** (`run_structured(goal, output=Model)` → `StructuredRun[Model]` via a schema-shaped answer tool, with an extraction pass for models that answer in prose anyway), **delegation** (`agent_tool` / `team_tools` — a specialist agent becomes a tool of another; the child inherits the parent's absolute deadline, `max_depth` stops A->B->A, artifacts surface namespaced as `<agent>/<name>`, and the child's trace nests under `StepKind.AGENT` via `AgentStep.children`), **autonomous loops** (`run_until` repeating until a predicate you wrote accepts, `refine` running generate-critique-revise with a second agent that approves only with the exact token `APPROVED`; `LoopResult`/`LoopIteration` keep every round), plus persistence that is opt-in (`InMemoryAgentRunSink` bounded buffer, `BaseAgentRunModel`/`make_agent_run_model`/`DbAgentRunSink`) and `make_agent_router` (`POST /run`, `/run/stream` SSE, artifact download) |
| `tempest_fastapi_sdk.api` | `register_exception_handlers`, `app_exception_handler`, OpenAPI error docs (`error_responses`, `raises`, `TempestAPIRouter`, `RaisesSpec`, `declared_raises`), `apply_cors`, `make_health_router`, `make_logs_router` (`GET` + `DELETE`), `resolve_log_files`, `make_prometheus_router`, `make_prometheus_registry`, `PrometheusMiddleware`, `BusinessMetrics`, `LogSource`, `DEFAULT_MAX_RECORDS_PER_FILE`, `make_tool_spec_router`, `make_token_dependency`, `make_bearer_token_dependency`, `make_jwt_user_dependency`, `make_role_dependency`, `make_permission_dependency`, `require_x_token`, `run_server`, `RequestIDMiddleware`, `RateLimitMiddleware` (+ `RateLimitStore`/`MemoryRateLimitStore`/`RedisRateLimitStore`/`RateLimitResult` and `key_by_ip`/`key_by_jwt_subject`/`key_by_jwt_claim`/`key_by_header`), `IdempotencyMiddleware`, `MemoryIdempotencyStore`, `RedisIdempotencyStore`, `BodySizeLimitMiddleware`, `AccessLogMiddleware` (one structured line per request), `HoneypotBanMiddleware` (+ `DEFAULT_HONEYPOT_PATTERNS`, `BanStore`/`MemoryBanStore`/`RedisBanStore`), `CSRFMiddleware`, `make_csrf_token_dependency`, `GracefulShutdownMiddleware`, `WebhookSignatureVerifier`, `RSAWebhookSignatureVerifier`, outbound `WebhookSender` (+ `WebhookDelivery`), OAuth2 (`GoogleOAuthClient`, `GitHubOAuthClient`, `OIDCProvider`), `HardenedStaticFiles`, `DEFAULT_STATIC_SECURITY_HEADERS`, `make_spa_router` (serve a compiled React/Vite SPA with a client-side fallback) + `DEFAULT_SPA_SECURITY_HEADERS` / `DEFAULT_SPA_CONTENT_SECURITY_POLICY` (its own CSP, which allows the bundle the SPA ships), `set_cookie`, `clear_cookie`, `SameSite`, `HealthCheck`, `setup_tracing` *(extra: `[otel]`)* |
| `tempest_fastapi_sdk.auth` *(extra: `[auth]`, opcional `[email]`, `[mfa]`, `[webauthn]`)* | `UserAuthService`, `make_auth_router`, `SignupSchema` / `LoginSchema` / `RefreshSchema` / `PasswordResetRequestSchema` / `PasswordResetConfirmSchema` + responses, `ActivationToken`, `PasswordResetToken`, email change/verify/recovery (`EmailChangeRequestSchema` / `EmailChangeConfirmSchema` / `EmailRecoveryRequestSchema` / `EmailChangeToken` / `EmailVerificationToken`, old-email security notice, opt-in `AUTH_EMAIL_RECOVERY_ENABLED`), MFA schemas (`MFAEnrollResponseSchema` / `MFAConfirmSchema` / `MFAVerifySchema` / `MFADisableSchema`), WebAuthn / passkeys (`WebAuthnService`, `BaseWebAuthnCredentialModel` / `make_web_authn_credential_model`, `Memory`/`RedisWebAuthnChallengeStore`, the six `/auth/webauthn/*` routes), opt-in DB-backed refresh tokens (`BaseUserRefreshTokenModel` / `make_user_refresh_token_model` / `LogoutSchema`) with rotation + reuse detection + `POST /auth/logout`, guards with a replaceable refusal (`require_admin(user, exception=UserIsNotAdminError)`, `make_flag_guard`), strict token-type checking through the dependency factories (`strict=` / `legacy_claims=`), one-active-link-per-flow (`AUTH_SINGLE_ACTIVE_TOKEN`), social login inside the same router (`AUTH_OAUTH_ENABLED` + `oauth_clients=` mounting `/auth/oauth/{provider}/login|callback` and `/auth/oauth/accounts[/unlink]`; `OAuthSettings`, `BaseUserOAuthAccountModel` / `make_user_oauth_account_model`, `NameMixin`, `OAuthAccountSchema` / `OAuthUnlinkSchema` — the callback issues through `issue_token_pair`, so a Google login is the same session as an email login), bilingual emails + backend pages resolved once per flow (`resolve_locale`: the link's `?lang=` via `stamp_locale` → `user.locale` → `Accept-Language` → `AUTH_DEFAULT_LOCALE`) — signup/activate/login/refresh/logout/reset + email change/recovery + TOTP 2FA out of the box; plus **Firebase ID token verification** (`[firebase]`) — `FirebaseAuth` (idempotent app init, `verify`, `get_identity` / `get_uid` / `get_optional_identity`), `FirebaseIdentity`, `FirebaseUserResolver[UserT]`, and one error `code` per failure (`FIREBASE_TOKEN_MISSING` / `_INVALID` / `_EXPIRED` / `_REVOKED`, `FIREBASE_USER_DISABLED` at 403, `FIREBASE_UNAVAILABLE`) |
...
| `tempest_fastapi_sdk.genai` *(extra: `[genai]`; capacity check imports without it)* | Self-hosted GenAI: capacity check (`probe_hardware`/`HardwareInfo`, `can_run`/`recommend`/`CapacityReport`, `estimate_model_bytes`/`fetch_num_params`, `ModelDtype`); local LLM `TextGenerator` (`generate`/`chat`/`stream`, typed `GenerationConfig`, int4/int8 quant, lazy load + idle unload); `Embedder` (+ `EmbeddingCache`/`InMemoryEmbeddingCache`/`RedisEmbeddingCache`/`AsyncEmbeddingCache`, `normalize=`) + `cosine_similarity`, `BatchScheduler` (coalesce concurrent calls), `ModelRegistry` (LRU model sharing, `.inventory()` / `.unload_idle()`); **runtime inventory** — `describe_model` / `runtime_report` → `LoadedModel` / `ModelRuntimeReport` (what is resident now, loaded-first and longest-idle-first, next to the host memory picture; reads attributes only, never triggers a load) + `GET /models`; `make_genai_router` (opt-in endpoints — `/generate`+SSE, `/chat`, `/embed`, `/rag`, `/transcribe`, `/tts`, `/image`); **image generation** (`[genai-image]`) — `ImageGenerator` (`generate` / `edit`) + `ImageGenerationConfig` + `GeneratedImage`; `resolve_device`/`auto_dtype_name`; **weight lifecycle** (`[genai-hub]`) — `ModelRef` (`revision=`/`local_files_only=`/`trust_remote_code=` uniform across every loader), `resolve_revision` (pin a branch to a commit sha), `download_model` (fetch ahead of the first request, refusing what the disk cannot hold) + `model_disk_bytes`, `list_cached_models`/`cache_size_bytes`/`remove_cached_model`; **tool calling** (`TextGenerator.chat_with_tools` / `OllamaGenerator.chat_with_tools`) + **structured output** (`generate_structured` — Ollama `format=`, transformers `lm-format-enforcer` via `[genai-structured]`; `parse_structured`); **`VisionTextGenerator`** (local VLM, `[genai-vlm]`); **`OnnxEmbedder`** (torch-free embeddings, `[genai-onnx]`); **generation cache** (`InMemoryGenerationCache`/`RedisGenerationCache`, deterministic-only) via `generation_cache=`; **token/context** (`count_tokens`/`count_message_tokens`/`truncate_messages`); **`GenAIMetrics`** (Prometheus request/latency/token metrics, `[prometheus]`); **moderation** (`RuleModerator`/`ClassifierModerator` + `ModerationResult`); Ollama backend (`[genai-ollama]`) — `OllamaGenerator`/`OllamaEmbedder` (HTTP, no torch) behind the `TextBackend` protocol, drop-in for `make_genai_router`/`Retriever`, plus vision (`generate(images=…)`); **hosted backend** (no extra) — `OpenAICompatGenerator` for any OpenAI-compatible `/chat/completions` (DeepSeek, Groq, OpenRouter, vLLM, TGI, Azure) behind the same `TextBackend`, `generate_with_usage` → `TokenUsage` (incl. `cache_hit_tokens`, read in both the DeepSeek and OpenAI spellings), `extra_body` for provider extensions; **JSON payloads** — `extract_json_list`/`extract_json_object`/`parse_structured_list` (balanced-span extraction, `None` instead of raising so a caller can retry) + `generate_structured_list` (retries at a rising temperature, `StructuredFormatError`); **usage accounting** — `BaseAIUsageModel` + `AIUsageStore` (one row per paid call: `record`/`record_duration`, `totals`/`by_service`/`per_day`/`top_subjects`, cost priced at read time, cached prefixes billed at `price_cache_hit_per_1k`); **`AIChatPipeline`** (memory recall → web-search augment → generate w/ bounded tool-calling loop → TTS → memory index) + `Tool` + `make_ai_chat_router` (`POST /chat` + `/chat/stream` SSE) |
| `tempest_fastapi_sdk.genai.rag` *(extra: `[genai-rag]`)* | RAG context for local LLMs: `WebSearch` (`.retrieve` one-shot search→extract→context) / `SearxngBackend` / `WebSearchBackend` + `SearchResult`, `ContentExtractor` (trafilatura, `.extract_many` concurrent), `PdfReader` (PyMuPDF) + `Document`/`Chunk`/`PdfPage`, `chunk_text`, `build_context`; corpus RAG — `Retriever` + `VectorStore`/`InMemoryVectorStore`/`PgVectorStore` (pgvector)/`ChromaVectorStore` (`[genai-chroma]`); **`Reranker`** (cross-encoder second stage) + **`HybridRetriever`** (BM25 + dense, `reciprocal_rank_fusion`, `rank-bm25`); long-term chat memory — `ChatMemory` (recency-aware, per-user, Chroma-backed) + `MemoryHit`; audio (`[genai-audio]`) — `SpeechToText` (faster-whisper, `beam_size`/`vad_filter`/`language_probability`, plus `batch_size` for batched decoding, `cpu_threads`/`num_workers`, `condition_on_previous_text` and `transcribe(on_progress=)`) + `TextToSpeech` (Coqui TTS) + `Transcription` |
| `tempest_fastapi_sdk.geo` *(extra: `[geo]`; offline layers import without it)* | Geolocation without a paid API: `haversine_km`, `estimate_travel` (offline road distance + per-mode time), offline geometry (`bounding_box`, `within_radius`, `nearest`, `initial_bearing`, `destination_point`, `point_in_polygon`, `polygon_area_km2`, `path_length_km`), DB radius search (`GeoPointMixin` + `GeoRepositoryMixin`/`PostGISRepositoryMixin`, `make_geo_point_model`) and SQL-side radius pagination (`GeoRepositoryMixin.paginate_nearby` → `NearbyMatch`, `haversine_distance_sql`, no PostGIS), routing (`OSRMBackend`/`RoutingBackend` — `route` + `route(with_geometry=True)` + `matrix`, per-mode profiles), geocoding (`NominatimBackend`/`GeocodingBackend`), polyline codec (`encode_polyline`/`decode_polyline`), Brazil (`uf_centroid`/`UF_CENTROIDS`, `cep_to_coordinate`, `extract_cep`, `resolve_br_coordinate` — CEP → address → state centroid); `Coordinate`/`TravelEstimate`/`TravelMode` (car/motorcycle/bus/bicycle/pedestrian)/`BoundingBox`/`GeocodeResult`/`DistanceMatrix` |
| `tempest_fastapi_sdk.hostbridge` *(no extra; `[pdf-read]` / `[pdf-layout]` for PDFs)* | Control the machine the service runs on, from WSL or Windows itself — `HostBridge` (power actions, host info, text read/write/list/delete, PDF pages, arbitrary PowerShell/`cmd`, the native file picker), `HostBridgeConfig` / `HostBridgeSettings`, `make_hostbridge_router` (auth required by signature, write side behind `destructive=True`), WSL ↔ Windows path translation resolved before it is checked against `allowed_base_paths`, and `HOST_UNAVAILABLE` (503) where there is no host to reach |
| `tempest_fastapi_sdk.openapi` | `generate_integration`, `load_spec`, `parse_spec`, `emit_schemas`, `emit_client`, `GenerationResult`, `default_output_dir`, `SpecError`, `SpecIR` / `SchemaIR` / `FieldIR` / `ClientIR` / `OperationIR` / `ParameterIR` — backs `tempest openapi-client`, which generates Pydantic schemas + a typed HTTP client from a third party's OpenAPI spec *(YAML needs the `[openapi]` extra)* |
| `tempest_fastapi_sdk.asyncapi` | `generate_stream`, `load_asyncapi_spec`, `parse_asyncapi`, `emit_stream`, `StreamGenerationResult`, `AsyncApiIR` / `StreamIR` / `ChannelIR` / `MessageIR` / `OperationIR` — backs `tempest asyncapi-client`, which generates a typed WebSocket client from an AsyncAPI 3 document: payload models plus two tagged unions, one for the frames you send and one for those you receive. OpenAPI cannot describe a socket, so this is a second document, not a second reader of the first. The document must declare `x-tempest-perspective` — `action` is relative to whoever published it, a client inverts every one, and a wrong inversion still compiles *(needs the `[websocket]` extra)* |
| `tempest_fastapi_sdk.integrations.messaging.zap` | `ZapClient` + the in-house WhatsApp gateway's 28 schemas and 27 operations (generated from the vendored spec, checked in, loaded lazily). A send answers `202` with the enqueued row, not a delivery — pass one `idempotency_key` per message so a retry answers `deduped: true` instead of sending twice. The status webhook has no schema in the specification yet, so it is not modelled |
| `tempest_fastapi_sdk.integrations.messaging.zap_ws` | `ZapStream` + the same gateway's WebSocket, generated from its AsyncAPI document. Six frames in two tagged unions — `SubscribeFrame` / `UnsubscribeFrame` / `SendFrame` out, `AckFrame` / `ErrorFrame` / `ServerMessageFrame` in — so a `match` over what arrives is exhaustive. A frame the document does not declare raises `ZapStreamFrameError` carrying its tag *(needs the `[websocket]` extra)* |
| `tempest_fastapi_sdk.integrations.payment.openpix` | `OpenPixClient` + all 686 OpenPix schemas and 125 operations (generated from the pinned spec, checked in, loaded lazily), plus `OpenPixEnvironment`, `OpenPixEvent` (28 webhook events), `make_openpix_webhook_dependency` / `webhook_verifier` / `decode_public_key` and `to_cents` / `reais_to_cents` / `cents_to_reais` |
...
| `tempest_fastapi_sdk.schemas` | `BaseSchema`, `BaseResponseSchema`, `ErrorResponseSchema`, `BasePaginationFilterSchema`, `BasePaginationSchema[T]`, `CursorPaginationFilterSchema`, `CursorPaginationSchema`, `LogEntrySchema`, `LogFilesClearedSchema`, `encode_cursor`, `decode_cursor`, `build_pagination_link_header` |
| `tempest_fastapi_sdk.services` | `BaseService` |
| `tempest_fastapi_sdk.settings` | `BaseAppSettings` (+ `AppSettingsMeta`), `ServerSettings`, `LogSettings`, `DatabaseSettings`, `RedisSettings`, `RabbitMQSettings`, `JWTSettings`, `CORSSettings`, `EmailSettings`, `UploadSettings`, `TokenSettings`, `WebPushSettings`, `TaskIQSettings`, `MinIOSettings`, `AuthSettings`, `OpenPixSettings` (AppID + ambiente; `openpix_kwargs()`) |
...
| `tempest_fastapi_sdk.ssr` *(extra: `[ssr]`)* | `Page` (typed component base), `html_response` (widget tree → `HTMLResponse`, full document or HTMX fragment), `make_htmx_router` (serves bundled HTMX locally, no CDN); `htmx()` / `aria()` / `data()` (typed builders for the `hx-*` / `aria-*` / `data-*` `attrs` map); `make_web_app_router` (serve a compiled static/wasm `tempestweb build` as a router, SPA history fallback), `build_web_app` (host a server-mode build as a WS/SSE sub-app, `theme=` handing a palette to every session's `App`, which the components its `view` builds resolve against), `detect_build_mode` |
| `tempest_fastapi_sdk.ui` *(extra: `[ssr]`)* | The interface layer, mirroring a service's `src/ui/`: `ui.pages` (`Page`), `ui.layout` (`Shell`, `Grid`), `ui.components` (`Card`, `Alert`, `DataTable`, `Pagination`, `EmptyState`, `NavBar`, `component_stylesheet`), `ui.forms` (`form_for`, `form_spec_for`, `render_form`, `parse_form`, `FormResult`, `form_stylesheet` — a form generated from a Pydantic schema and read back into it), `ui.css` (`StyleSheet`, `Rule`, `Media`, `ThemeTokens`, `cls`, `make_css_router`), `app_stylesheet` |
...
- `orderable_columns` / `max_page_size` (class attributes on the filter schema, and on `BaseRepository` as a second line) close the sort oracle: `order_by` outside the declared set is a 422 `ORDER_BY_NOT_ALLOWED` whose `details["allowed"]` lists only that set, and `page_size` is capped (default 100, `DEFAULT_MAX_PAGE_SIZE`).
...
class UserFilterSchema(BasePaginationFilterSchema):
    """Public user listing: sortable by name or creation date only."""
...
| `CPFField`, `CNPJField`, `CPFOrCNPJField`, `PhoneBRField`, `MobilePhoneBRField`, `CEPField`, `is_valid_*`, `normalize_*`, `parse_phone_br`, `only_digits` | [BR document & phone validation recipe](#br-document--phone-validation-recipe) |
...
| `parse_phone_br` | `(str)` to `PhoneNumberBR` or `None` | Splits into area code, number, `is_mobile` and E.164. |
...
| `CPFField`, `CNPJField`, `CPFOrCNPJField`, `PhoneBRField`, `MobilePhoneBRField`, `CEPField` | `Annotated[str, AfterValidator(...)]` | Drop-in Pydantic field types — validate + normalize automatically. The old names without the `Field` suffix (`CPF`, `CNPJ`, …) remain as deprecated aliases since v0.76. |
