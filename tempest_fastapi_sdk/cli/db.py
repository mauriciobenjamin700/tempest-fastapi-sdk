...
def db_check(
    ...
    """Fail when the models drifted away from the migration tree.

    ``tempest db revision --autogenerate`` records a diff; nothing
    notices when a developer edits a model and forgets to. This runs
    the same comparison and reports the operations a revision would
    contain, so CI fails on the commit that introduced the drift
    instead of on the deploy that needed the column.

    The database has to be at head for the comparison to mean anything —
    Alembic says so itself, and the message is passed through.

    Raises:
        typer.Exit: Code 1 when the models and the migrations disagree.
    """
    helper = _helper(ini, _resolve_database_url(database_url))
    ...
    with _alembic_errors():
        try:
            command.check(cast("Any", helper).config)
            ...
            raise typer.Exit(1) from exc
    typer.echo("No drift: the models match the migration tree.")
