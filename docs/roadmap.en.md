...
| **ONNX and HuggingFace quantization** | ✅ v0.173 | `quantize_onnx_dynamic`, `quantize_onnx_static` (calibration reader from any iterable of feeds), and the HF path through `optimum`: `export_hf_to_onnx`, `optimize_hf_onnx`, `quantize_hf_onnx` — plus `quantize_hf_bnb` (int4/int8 in PyTorch). |
