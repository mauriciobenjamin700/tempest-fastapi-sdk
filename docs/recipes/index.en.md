...
| **[Pix protocol (one contract, many providers) »](pix-protocol.md)** | `PixProvider` (Protocol), `PixCharge` / `PixChargeRequest` / `PixPayer`, canonical `PaymentStatus` beside the raw `provider_status`, `PixPaymentEvent`, `OpenPixPixProvider` — the same service switching provider in one line |
