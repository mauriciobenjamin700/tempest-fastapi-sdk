...
| **[Protocolo de Pix (um contrato, vários provedores) »](pix-protocol.md)** | `PixProvider` (Protocol), `PixCharge` / `PixChargeRequest` / `PixPayer`, `PaymentStatus` canônico ao lado do `provider_status` cru, `PixPaymentEvent`, `OpenPixPixProvider` — o mesmo service trocando de provedor por uma linha |
