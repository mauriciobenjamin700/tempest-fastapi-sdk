...
async def test_the_agent_recovers_from_a_failing_tool() -> None:
    ...
    run = await Agent(backend, tools=[text_tool("save", "Save it.", save)]).run("save it")

    assert run.succeeded is True
    assert "disk full" in failed_steps(run)[0].error
```
...
invoicing = Skill(
    ...
    instructions="The full NF-e guide goes here.",
    tools=[get_weather],
)
...
async def test_the_whole_script_was_used() -> None:
    ...
    backend = ScriptedBackend([replies("done")])

    await Agent(backend).run("hi")
