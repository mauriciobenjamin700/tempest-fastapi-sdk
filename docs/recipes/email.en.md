...
!!! warning "STARTTLS vs SSL/TLS — don't force it against a plain server"
    Port **587** → `SMTP_USE_TLS=true` (default): connect in clear text and
    upgrade via STARTTLS. Port **465** → `SMTP_USE_SSL=true`: connect
    already encrypted. **A plain SMTP server (MailHog on `:1025`, or
    `:25`)** doesn't speak STARTTLS — leave **`SMTP_USE_TLS=false`** and
    `SMTP_USE_SSL=false`, otherwise aiosmtplib raises `SMTPException: SMTP
    STARTTLS extension not supported by server.` The `.env.example`
    generated by `tempest new` with the `[email]` extra already ships both
    turned off for MailHog.
